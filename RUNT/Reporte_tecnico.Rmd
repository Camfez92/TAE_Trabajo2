---
title: "CANTIDAD DE VEHÍCULOS REGISTRADOS EN EL RUNT PERIODO 2012-2018"
author: "Equipo 7"
...
plot(tseries,
     main = "Número de vehículos registrados por mes",
...
g4 <- ggplot(dbMes, aes(x = (mesab), y = (Unidades))) +
      ...
      theme_light() + 
      ggtitle("Promedio mensual de vehículos registrados en el RUNT") +
...
plot(modelo.knn.grilla,
     main = "Raíz del error cuadrático medio vs. número de vecinos",
     xlab = "Número de vecinos (k) \n: Figura 6: K óptimo por grafica de codo",
     ylab = "Raíz del error cuadrático medio (RMSE)")
```

A continuación se muestra una gráfica de predichos vs observados con el KNN seleccionado:
...
plot <- ggplot(data = resultado_plot,
         ...
         color = curva)) +
    geom_line() +
    ggtitle("Datos observados vs predichos entre 2012 y 2017 con KNN") +
...
Otro modelo que se desea probar es una red neuronal en python. Para ello, se se utiliza 75% de los datos para su entrenamiento y 25% para validación y por supuesto, se normalizan. El código de la red neuronal se encuentra en el repositorio de Github en un archivo titulado "NN" en la carpeta RUNT. Luego de entrenarlo, se llegó a un $R^2$ de 0.84 para entrenamiento y de 0.74 para validación. Esta es la gráfica que muestra su comportamiento para los datos entre 2012 y 2017:
...
plot <- ggplot(data = resultado_plot,
         ...
         color = curva)) +
    geom_line() +
    ggtitle("Datos observados vs predichos entre 2012 y 2017 con red neuronal") +
...
```{r}
# Predicción con KNN con KNN con preprocesamiento y k = 5
...
```{r}
g1 <- ggplot(pred.knn, aes(x = as.Date(Fecha), y = Unidades)) +
      geom_line(size = 0.1, color = "#4d85ea") +
      ggtitle("Número de vehículos registrados en el RUNT¹ por día",
              subtitle = "¹ Registro Único Nacional de Tránsito") +
      xlab("Fecha [AAAA-MM-DD]") +
